...
// added
// test if bounding box intersect with view frustrum
bool vf_instersect(Vector3 &localpoint, Matrix4x4 transform, Events::Event_GATHER_DRAWCALLS* pDrawEvent) {
	Vector3 globalpoint = transform * (localpoint * 0.01f);
	return ((globalpoint - pDrawEvent->nf_p_gbl) * pDrawEvent->nf_n_gbl < 0.f)
		&& ((globalpoint - pDrawEvent->ff_p_gbl) * pDrawEvent->ff_n_gbl < 0.f)
		&& ((globalpoint - pDrawEvent->lf_p_gbl) * pDrawEvent->lf_n_gbl < 0.f)
		&& ((globalpoint - pDrawEvent->rf_p_gbl) * pDrawEvent->rf_n_gbl < 0.f)
		&& ((globalpoint - pDrawEvent->tf_p_gbl) * pDrawEvent->tf_n_gbl < 0.f)
		&& ((globalpoint - pDrawEvent->bf_p_gbl) * pDrawEvent->bf_n_gbl < 0.f);
}
// end added
...
void SingleHandler_DRAW::do_GATHER_DRAWCALLS(Events::Event *pEvt)
{
	...
	for (int iInst = 0; iInst < pMeshCaller->m_instances.m_size; ++iInst){
		PositionBufferCPU* currpbcpu = pMeshCaller->m_instances[iInst].getObject<MeshInstance>()->m_hAsset.getObject<Mesh>()->m_hPositionBufferCPU.getObject<PositionBufferCPU>();

		SceneNode* currscenode = pMeshCaller->m_instances[iInst].getObject<MeshInstance>()->getFirstParentByTypePtr<SceneNode>();

		if (currscenode) {
			DebugRenderer::Instance()->createBoundingBoxMesh(currpbcpu->dim1_min, currpbcpu->dim1_max,
			currpbcpu->dim2_min, currpbcpu->dim2_max,
			currpbcpu->dim3_min, currpbcpu->dim3_max, currscenode->m_worldTransform);
		}
	...
	if (pMeshCaller->m_performBoundingVolumeCulling) {
		...
		if (pDrawEvent) {
			for (int iInst = 0; iInst < pMeshCaller->m_instances.m_size; ++iInst)
			{
				...
				PositionBufferCPU* currpbcpu = pInst->m_hAsset.getObject<Mesh>()->m_hPositionBufferCPU.getObject<PositionBufferCPU>();
				SceneNode* currscenode = pInst->getFirstParentByTypePtr<SceneNode>();
				PrimitiveTypes::Float32 d1min = currpbcpu->dim1_min, d1max = currpbcpu->dim1_max,
					d2min = currpbcpu->dim2_min, d2max = currpbcpu->dim2_max,
					d3min = currpbcpu->dim3_min, d3max = currpbcpu->dim3_max;
				...
				if (!pInst->hw3_cull || vf_instersect(A, currscenode->m_worldTransform, pDrawEvent)
					|| vf_instersect(B, currscenode->m_worldTransform, pDrawEvent)
					|| vf_instersect(C, currscenode->m_worldTransform, pDrawEvent)
					|| vf_instersect(D, currscenode->m_worldTransform, pDrawEvent)
					|| vf_instersect(E, currscenode->m_worldTransform, pDrawEvent)
					|| vf_instersect(F, currscenode->m_worldTransform, pDrawEvent)
					|| vf_instersect(G, currscenode->m_worldTransform, pDrawEvent)
					|| vf_instersect(H, currscenode->m_worldTransform, pDrawEvent)) {
					pInst->m_culledOut = false;
					++pMeshCaller->m_numVisibleInstances;
					//PEINFO("vf_intersect A: %d", vf_instersect(A, currscenode->m_worldTransform, pDrawEvent));
				}
				else {
	...
	PEINFO("drawevt: %ull, NUM_VIS_INST: %d", pDrawEvent, pMeshCaller->m_numVisibleInstances);

...
void PositionBufferCPU::ReadPositionBuffer(const char *filename, const char *package)
{
	...
	for (int i = 0; i < n * 3; i++)
	{
		...
		// added
		if (i % 3 == 0) {
			dim1_min = std::min(dim1_min, val);
			dim1_max = std::max(dim1_max, val);
		}
		else if (i % 3 == 1) {
			dim2_min = std::min(dim2_min, val);
			dim2_max = std::max(dim2_max, val);
		}
		else {
			dim3_min = std::min(dim3_min, val);
			dim3_max = std::max(dim3_max, val);
		}
	}
}

    ...
    while (!gqh.getObject<Events::EventQueue>()->empty())
    {
        ...
        else if (Event_CALCULATE_TRANSFORMATIONS::GetClassId() == pGeneralEvt->getClassId())
        {
            ...
            // Push DRAW event into message queue because camera has updated transformations
            {
				...
				// added
				drawEvt->nf_n_gbl = pcam->nf_n_gbl;
				drawEvt->nf_p_gbl = pcam->nf_p_gbl;

				drawEvt->ff_n_gbl = pcam->ff_n_gbl;
				drawEvt->ff_p_gbl = pcam->ff_p_gbl;

				drawEvt->lf_n_gbl = pcam->lf_n_gbl;
				drawEvt->lf_p_gbl = pcam->lf_p_gbl;

				drawEvt->rf_n_gbl = pcam->rf_n_gbl;
				drawEvt->rf_p_gbl = pcam->rf_p_gbl;

				drawEvt->tf_n_gbl = pcam->tf_n_gbl;
				drawEvt->tf_p_gbl = pcam->tf_p_gbl;

				drawEvt->bf_n_gbl = pcam->bf_n_gbl;
				drawEvt->bf_p_gbl = pcam->bf_p_gbl;
				// end added

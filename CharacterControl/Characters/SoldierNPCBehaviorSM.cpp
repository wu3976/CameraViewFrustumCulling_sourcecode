...
// sent by mov state machine when it reach target
	...
	if (m_state == PATROLLING_WAYPOINTS)
	{
		...
		if (pGameObjectManagerAddon)
		{
			...
			if (pWP && StringOps::length(pWP->m_nextWayPointName) > 0)
			{
				...
				if (pWP->m_num_waypts == 1) {
					pWP = pGameObjectManagerAddon->getWayPoint(pWP->m_nextWayPointName);
				}
				else if (pWP->m_num_waypts == 2) {
					if (rand() % 2) {
						pWP = pGameObjectManagerAddon->getWayPoint(pWP->m_nextWayPointName);
					}
					else {
						pWP = pGameObjectManagerAddon->getWayPoint(pWP->m_next2WayPointName);
					}
				} else if (pWP->m_num_waypts == 3){
					int randrem = rand() % 3;
					if (!randrem) {
						pWP = pGameObjectManagerAddon->getWayPoint(pWP->m_nextWayPointName);
					}
						...
						pWP = pGameObjectManagerAddon->getWayPoint(pWP->m_next3WayPointName);
					}
				}
				else {
					assert("m_num_waypts invalid!" == NULL);
				}
				//pWP = pGameObjectManagerAddon->getWayPoint(pWP->m_nextWayPointName);
				if (pWP)
				{
					...
					pEvt->m_running = pWP->m_needToRunToThisWaypoint; // 50% it is running

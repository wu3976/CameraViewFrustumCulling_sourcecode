    ...
    if (spawnALotOfMeshes)
    {
        for (int ix = 0; ix < maxX; ++ix)
        {
            for (int iy = 0; iy < 10; ++iy)
            {
				...
				pImrodMeshInst->hw3_cull = true;
				// added
				pImrodMeshInst->scenenode_for_hw3 = pMainSN;
				//end
                pImrodMeshInst->addDefaultComponents();
				pImrodMeshInst->initFromFile("imrod.x_imrodmesh_mesh.mesha", "Default", m_pContext->    m_gameThreadThreadOwnershipMask, pMainSN->m_base, true);

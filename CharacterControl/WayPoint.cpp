...
int Event_CREATE_WAYPOINT::l_Construct(lua_State* luaVM)
{
	...
	int needToRun = (int)lua_tonumber(luaVM, -numArgs--),
		// these are added
		num_waypts = (int)lua_tonumber(luaVM, -numArgs--);
	const char* next2WayPointName = lua_tostring(luaVM, -numArgs--),
		* next3WayPointName = lua_tostring(luaVM, -numArgs--);

	pEvt->m_needToRunToThisWaypoint = needToRun;
	// these are added
	pEvt->m_num_waypts = num_waypts;
	StringOps::writeToString(next2WayPointName, pEvt->m_next2WayPointName, 32);
	StringOps::writeToString(next3WayPointName, pEvt->m_next3WayPointName, 32);
...
: Component(context, arena, hMyself)
{
	...
	// added
	m_num_waypts = pEvt->m_num_waypts;
	StringOps::writeToString(pEvt->m_next2WayPointName, m_next2WayPointName, 32);
	StringOps::writeToString(pEvt->m_next3WayPointName, m_next3WayPointName, 32);
	m_base = pEvt->m_base;
}
